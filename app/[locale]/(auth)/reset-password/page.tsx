...
export default function ResetPasswordPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.password) {
      ...
      newErrors.password = t('auth.password8Chars');
  ...
  const passwordRequirements = [
    { text: t('auth.atLeast8Chars'), met: formData.password.length >= 8 },
    { text: t('auth.oneUppercase'), met: /[A-Z]/.test(formData.password) },
    { text: t('auth.oneLowercase'), met: /[a-z]/.test(formData.password) },
    { text: t('auth.oneNumber'), met: /\d/.test(formData.password) },
  ];
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {formData.password && (
              <div className="bg-background border border-primary/20 rounded-lg p-3">
                <p className="text-gray-400 text-xs mb-2">{t('auth.passwordRequirements')}</p>
